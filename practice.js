...
function divider(numbersArray){
  let gucciEven = []
  let gucciOdd = []
  let gucciGang = [gucciEven,gucciOdd]
  for (i=0; i<numbersArray.length; i++){
    if (numbersArray[i]%2 === 0){
      gucciEven.push(numbersArray[i])
    }
    else {
      gucciOdd.push(numbersArray[i])
    }
  }
  return gucciGang
}
...
function removeItem(myGroceryList, remove){
  for(i=0; i<myGroceryList.length; i++){
    if (myGroceryList[n] = remove){
      myGroceryList.splice(i,1)
    }
  }
  if (myGroceryList = false){
    myGroceryList.splice(0)
  }
...
for (i=0; i < numbers.length; i++){
    newNumbers.push(numbers[i] = parseInt(numbers[i]) + 10);
  }
  return newNumbers
}
